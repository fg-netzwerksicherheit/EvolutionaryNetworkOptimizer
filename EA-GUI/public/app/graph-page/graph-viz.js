...
/**
 * @class angular_module.graphPage
 * @memberOf angular_module
 * @description This component takes a graphML-file and will visualize it.
 */
angular.module('graphPage')
    .component('graphViz', {
        templateUrl: 'app/graph-page/graph-viz.html',
        controller: function () {
          ...
          $('#eventText')[0].style.visibility = 'hidden';
          $('#toggleEvent')[0].style.visibility = 'hidden';
          ...
          function load(e) {
              ...
              $('#event')[0].style.visibility = 'visible';
              $('#eventText')[0].style.visibility = 'visible';
          }
          ...
          $('#event')[0].addEventListener('change', function(evt) {
             var eventFiles = evt.target.files;
             for (var i = 0, f; f = eventFiles[i]; i++) {
               var eventReader = new FileReader();
               eventReader.onload = (function (theFile) {
                 return function (e) {
                     var events = JSON.parse(e.target.result);
                     events = new Sorter().quickSort(events.events, 0, events.events.length-1);
                     draw(network, events, data);
                     $('#eventText')[0].innerHTML = "Number of Events: " + events.length;
                     $('#toggleEvent')[0].style.visibility = 'visible';
                 }
               })(f);
               eventReader.readAsText(f);
             }
          }, false);
