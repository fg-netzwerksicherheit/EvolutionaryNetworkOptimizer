...
CPP_SRCS := $(shell find ../src -name "*.cpp" -not -path "../src/test" -not -path "../src/test/*")
OBJS := $(CPP_SRCS:../%.cpp=./%.o)
CPP_DEPS := $(shell find . -name "*.d")
SUB_DIRS := $(shell cd .. && find ./src -type d -not -path "./src/test" -not -path "./src/test/*")

TARGET := NetworkOptimizer-debug

# dependencies
INCLUDE_PATH := -I"../../Simulation/src/" -I"../../AnyGraph/src/"
LIB_PATH := -L/usr/local/lib
LIBS := -lSimulation-debug -lAnyGraph-debug -lopenea -lns3.26-core-debug -lboost_unit_test_framework
